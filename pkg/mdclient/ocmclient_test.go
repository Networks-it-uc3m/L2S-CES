...
func TestAllocateMonitoringNodeIPs(t *testing.T) {
	...
	expected := map[string]net.IPNet{
		"cluster-a": {
			IP:   net.ParseIP("10.42.0.1").To4(),
			Mask: net.CIDRMask(29, 32),
		},
		"cluster-b": {
			IP:   net.ParseIP("10.42.0.2").To4(),
			Mask: net.CIDRMask(29, 32),
		},
		"cluster-c": {
			IP:   net.ParseIP("10.42.0.3").To4(),
			Mask: net.CIDRMask(29, 32),
		},
	}

	for clusterName, wantIPNet := range expected {
		gotIPNet, ok := allocated[clusterName]
		if !ok {
			t.Fatalf("missing allocation for cluster %q", clusterName)
		}

		if !reflect.DeepEqual(gotIPNet, wantIPNet) {
			t.Fatalf("cluster %s got IPNet %v, want %v", clusterName, gotIPNet, wantIPNet)
		}
	}
}
